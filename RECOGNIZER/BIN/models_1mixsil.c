...
int main(int argc, char *argv[])
{
	...
	{    
	 	...
	 	while(CAT==FALSE)  
		{
	  		...
	  		{	
				if(i==1) fprintf(outfile,"~h \"one\"");
				if(i==2) fprintf(outfile,"~h \"two\"");
				if(i==3) fprintf(outfile,"~h \"three\"");
				if(i==4) fprintf(outfile,"~h \"four\"");
				if(i==5) fprintf(outfile,"~h \"five\"");
				if(i==6) fprintf(outfile,"~h \"six\"");
				if(i==7) fprintf(outfile,"~h \"seven\"");
				if(i==8) fprintf(outfile,"~h \"eight\"");
				if(i==9) fprintf(outfile,"~h \"nine\"");
				if(i==10) fprintf(outfile,"~h \"oh\"");
				if(i==11) fprintf(outfile,"~h \"zero\"");
				if(i==12) { silmodel(fp,outfile);
					     break; }
				CAT=TRUE;
			}
		}
